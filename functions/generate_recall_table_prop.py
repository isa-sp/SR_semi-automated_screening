...
def generate_recall_table_prop(evaluation, props, n_simulations, data_type = 'original'):
    ...
    for key, value in evaluation.items():
        ...
        if data_type == 'adapted':
            df_sim = pd.DataFrame(list(zip(review, seed, total_records, rel_records, sim_number, train_model, feature_model, query_model, simulations, props, recalls)),columns = ['Review', 'Seed', 'Total records', 'Relevant records', 'Simulation number', 'Train model', 'Feature model', 'Query model', 'Simulation', 'percentage of records screened','recall'])
